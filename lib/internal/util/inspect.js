...
function defineColorGetter({ style, codes, enumerable }) {
  function color(...strings) {
    ...
    if (ansiCodeStart === -1) {
      ...
      let middle = string.slice(ansiCodeStart - 1, string.length);

      for (const code of ansiCodes) {
        result += code.start;
        middle = replaceCloseCode(middle, code.end, code.start, code.isDimOrBold);
      }
    ...
    if (!lazyUtilColors().shouldColorize(process.stderr)) return string;

    return result;
  }

  ObjectDefineProperty(colorize, style, {
    __proto__: null,
    get: function() {
      ...
      const boundColor = color.bind(context);
      ObjectSetPrototypeOf(boundColor, colorize);
...
const ansiBoldCode = 1;
const ansiDimCode = 2;


for (const [style, descriptor] of ObjectEntries(ObjectGetOwnPropertyDescriptors(inspect.colors))) {
  ...
  defineColorGetter({
    ...
    codes: {
      ...
      isDimOrBold: value[0] === ansiBoldCode || value[0] === ansiDimCode,
    },
